...
inline std::istream& operator>>(std::istream &is, mpreal& v)
{
	...
	// TODO: rewrite... This sort of works now, although it doesn't really
	//       handle incorrect inputs well. It should work for correct inputs
	//       though.
    std::string tmp;
	std::vector<char> tmp_chars;
	bool is_number = true;

	bool exponent_has_appeared = false;
	bool just_been_exponent = false;
	char tmp_char;
	while (is_number) {
		if (!is.get(tmp_char)) {
			...
			switch (tmp_char) {
				case 'e':
					if (exponent_has_appeared) {
						goto GOTO_default;
					} else {
						just_been_exponent = true;
						is_number = true;
						tmp_chars.push_back(tmp_char);
					}
					break;
				case '0': case '1': case '2': case '3': case '4': case '5':
				case '6': case '7': case '8': case '9':
					just_been_exponent = false;
					is_number = true;
					tmp_chars.push_back(tmp_char);
					break;
				case '.':
